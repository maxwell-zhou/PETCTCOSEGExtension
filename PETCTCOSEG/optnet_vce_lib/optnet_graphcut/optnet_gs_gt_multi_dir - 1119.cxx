                                          ...
                                          
{
   ...
   weight = pow( (k+1), pow_vce )-2 * pow( k, pow_vce) + pow( (k-1), pow_vce) ;
        ...
        for (i1 = 0; i1 < s1; ++i1) {
            for (i0 = 0; i0 < s0 - 1; ++i0) {
				...
				for (i2 = s2 - 1 - upBound; i2 >= lowBound; --i2) {
					...
					m_graph.add_arc(i2,          i0,          i1,          i3,		i2 + shape_vce.meanDir0[i0][i1] - shape_vce.lowDir0[i0][i1],      i0 + 1,          i2,		i3 );//Hard constraint
...
optnet_gs_gt_multi_dir<_Cost, _Cap, _Tg>::build_graphcut_arcs()
{
	...
	//int coef = 1000000;
	//int coef = 1;
	float theta = 0.4;   //For lymph nodes
	//float theta = 1;
	//int coef = 1;
